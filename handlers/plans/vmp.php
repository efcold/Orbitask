...
$newPlanRef = $database->getReference("users/{$uid}/plans/{$planId}");
$newPlanRef->update([
    "banner"     => $banner,    
]);
header('Location: ' . $_SERVER['REQUEST_URI']);
        ...
        if (!empty($_FILES['task_files']['tmp_name']) && is_array($_FILES['task_files']['tmp_name'])) {
            ...
            foreach ($_FILES['task_files']['tmp_name'] as $i => $tmpPath) {
                if (is_uploaded_file($tmpPath) && $_FILES['task_files']['error'][$i] === UPLOAD_ERR_OK) {
                    $orig     = preg_replace('/[^a-zA-Z0-9_\-\.]/', '_',
                                    basename($_FILES['task_files']['name'][$i]));
...
if (isset($_POST['edit_task'])) {
    $planId   = $_POST['plan_id'];
    $taskId   = $_POST['task_id'];
    $taskRef      = $database->getReference("users/{$uid}/plans/{$planId}/tasks/{$taskId}");
...
if (isset($_POST['delete_task'])) {
    ...
    $database
      ->getReference("users/{$uid}/plans/{$planId}/tasks/{$taskId}")
      ->remove();
...
if (isset($_POST['assign_task'])) {
    ...
    $viewUrl      = "https://orbitask.site/viewplan.php?plan_id={$planId}&owner_uid={$uid}";
...
if (isset($_POST['invite_user'])) {
    ...
    $planTitle = $database
    ->getReference("users/{$uid}/plans/{$planId}/title")
    ->getValue() ?? 'your plan';
    ...
    if (!$alreadyInvited) {
        ...
        try {
            ...
            $mail->isHTML(true);
            $mail->Subject = 'You have been invited to "' . $planTitle . '"';
...
if (isset($_POST['remove_invite'])) {
    ...
    $database
        ->getReference("users/{$uid}/plans/{$planId}/invited/{$removeKey}")
        ->remove();
...
if (isset($_POST['add_announcement'])) {
    ...
    $announcementRef = $database
        ->getReference("users/{$uid}/plans/{$planId}/announcements")
        ->push();
  ...
  notifyInvited($uid, $planId, $subject, $html);
    header('Location: ' . $_SERVER['REQUEST_URI']);
    exit();
}
...
if (isset($_POST['delete_announcement'])) {
    ...
    $annRef = $database
        ->getReference("users/{$uid}/plans/{$planId}/announcements/{$announcementId}");
    $annRef->remove();
...
if (isset($_POST['edit_announcement'])) {
    ...
    $annRef = $database->getReference("users/{$uid}/plans/{$planId}/announcements/{$announcementId}");
